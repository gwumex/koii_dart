...
void main() {
  ...
  group('Test commitment', () {
    ...
    test('Get token accounts by owner', () async {
      ...
      final createdAccount = await koiiClient.createAssociatedTokenAccount(
  ...
  group('Test informational methods', () {
    ...
    test('Call to getVersion() succeeds and parses the response correctly',
      ...
      expect(version.koiiCore, isNotNull);
      expect(version.koiiCore.codeUnitAt(0), equals(49));
      expect(version.koiiCore.codeUnitAt(1), equals(46));
    });
...
Future<SplToken> _createToken({
  ...
  final koiiClient = createTestKoiiClient();
  // Now we have SOL to create the token
  final splToken = await koiiClient.initializeMint(
    decimals: 2,
  ...
  final supplyAccount = await koiiClient.createAssociatedTokenAccount(
  ...
  final associatedAccount = await koiiClient.getAssociatedTokenAccount(
    mint: splToken.mint,
  ...
  await koiiClient.transferSplToken(
